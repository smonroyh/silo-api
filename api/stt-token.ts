...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (authError || !user) {
      console.error("Auth validation failed:", authError?.message || "No user found for this token");
    ...
    if (!apiKey) {
      console.error("No DEEPGRAM_API_KEY set in Vercel Environment Variables");
    ...
    if (!tokenResp.ok) {
      ...
      console.error("Error creating Deepgram auth grant:", errorText);
    ...
    console.error("Unhandled error in /api/stt-token:", error.message);
